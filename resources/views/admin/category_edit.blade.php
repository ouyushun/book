...
<form action="" method="post" class="form form-horizontal" id="form-category-edit">
  ...
  <div class="row cl">
    ...
    <div class="formControls col-5">
      <input type="text" class="input-text" value="{{$category->cate_name}}" placeholder="" name="name" datatype="*" nullmsg="名称不能为空">
    </div>
    <div class="col-4"> </div>
  </div>
  <div class="row cl">
    ...
    <div class="formControls col-5"> <span class="select-box" style="width:150px;">
      <select class="select" name="parent_id" size="1">
        ...
        @foreach($categories as $temp)
          @if($category->parent_id == $temp->cate_id)
            <option selected value="{{$temp->cate_id}}">{{$temp->cate_name}}</option>
          @else if($category->cate_id != $temp->cate_id)
