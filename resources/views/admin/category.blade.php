			...
			@foreach($categories as $category)
				<tr class="text-c">
					...
					<td>@if($category->parent != null)
						{{$category->parent->cate_name}}
					@endif</td>
					<td>@if($category->preview != null)
						<img src="{{$category->preview}}" alt="" style="width: 50px; height: 50px;">
					@endif</td>
					<td class="td-manage">
						<a title="编辑" href="javascript:;" onclick="category_edit('编辑类别','/admin/category/edit?id={{$category->cate_id}}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
						<a title="删除" href="javascript:;" onclick='category_del("{{$category->cate_name}}", "{{$category->cate_id}}")' class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
					</td>
				</tr>
			@endforeach
...
<script type="text/javascript">
	...
	function category_del(name, id) {
        ...
        url: '/admin/service/category/delete', // 需要提交的 url
        dataType: 'json',
        data: {
          id: id,
          _token: "{{csrf_token()}}"
